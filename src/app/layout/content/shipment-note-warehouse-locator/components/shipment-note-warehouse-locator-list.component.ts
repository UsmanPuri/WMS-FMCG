...
export class ShipmentNoteWarehouseLocatorListComponent extends BaseComponent {
...
@ViewChild('PalletBarcodeModal') PalletBarcodeModal: any; // Naye Modal ka reference
palletBarcodeList: any[] = []; 
private palletModalRef: any; 
  ...
  constructor(injector: Injector, private _shipmentNoteLocatorService: ShipmentNoteLocatorService,
   private ngxService: NgxUiLoaderService, private chRef: ChangeDetectorRef, private modalService: ModalManager,) {
    super(injector);
  }
  ...
  Details(id) {
    debugger
    let updateUrl: string = this.routesList.SHIPMENT_NOTE_WAREHOUSE_LOCATOR_EDIT + `/${id}`;
    this.router.navigate([updateUrl]);
  }

  openPalletBarcodeModal(shipmentNoteId: any) {
  this.ngxService.start();
  
  // Dono zaroori parameters jo backend demand kar raha hai
  let companyBusinessUnitInfo: any = {
    CompanyId: this.localStorageService.getCompanyId(),
    BusinessUnitId: this.localStorageService.getBusinessUnitId(), // Safe side ke liye backup
  ...
  this._shipmentNoteLocatorService.getKeyPair(apiAddress, companyBusinessUnitInfo)
    .subscribe(res => {
      debugger
      this.palletBarcodeList = res as any[]; // Saare pallets array mein save ho gaye
      this.ngxService.stop();

      // Modal ko zinda (Open) karna
      this.palletModalRef = this.modalService.open(this.PalletBarcodeModal, {
        size: "lg",
        backdrop: true,
        animation: true,
        keyboard: true,
